      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="mt-16 border-t border-gray-100 pt-12 text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            {[{name:"DoorDash",url:RESTAURANT_INFO.social.doordash,color:"bg-red-500"},{name:"GrubHub",url:RESTAURANT_INFO.social.grubhub,color:"bg-orange-500"},{name:"Uber Eats",url:RESTAURANT_INFO.social.ubereats,color:"bg-green-600"}].map((app) => (
              <a key={app.name} href={app.url} target="_blank" rel="noopener noreferrer"
                className={`${app.color} text-white px-6 py-2.5 rounded-full text-sm font-semibold hover:opacity-90 transition-opacity`}>{app.name}</a>
            ))}
          </div>
        </div>
      </div>
          ...
          <div className="w-full max-w-sm bg-white h-full flex flex-col fade-in">
                ...
                <div key={item.id} className="flex gap-4 items-start">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <button onClick={() => updateQty(item.id, -1)} className="w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center hover:bg-gray-200"><Minus className="w-3 h-3" /></button>
                      <span className="text-sm font-bold w-4 text-center">{item.quantity}</span>
                      <button onClick={() => updateQty(item.id, 1)} className="w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center hover:bg-gray-200"><Plus className="w-3 h-3" /></button>
                    </div>
                  </div>
            ...
            <div className="px-5 py-4 border-t border-gray-100">
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                {[{name:"DoorDash",url:RESTAURANT_INFO.social.doordash,bg:"bg-red-500"},{name:"GrubHub",url:RESTAURANT_INFO.social.grubhub,bg:"bg-orange-500"},{name:"Uber Eats",url:RESTAURANT_INFO.social.ubereats,bg:"bg-green-600"}].map((app) => (
                  <a key={app.name} href={app.url} target="_blank" rel="noopener noreferrer"
                    className={`${app.bg} text-white py-2 rounded-full text-xs font-semibold text-center hover:opacity-90 transition-opacity`}>{app.name}</a>
                ))}
              </div>
              <button onClick={clearCart} className="w-full text-gray-400 hover:text-gray-600 text-xs mt-1 py-1 transition-colors">Clear cart</button>
            </div>
          </div>
        </div>
      )}
